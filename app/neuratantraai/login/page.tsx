...
export default function NeuraTantraAILogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/login', {
        ...
        body: JSON.stringify({ passcode, role: 'neuratantraai' }),
      });
      ...
      <div className="w-full max-w-md rounded-lg border border-stone-200 bg-white p-8 shadow-sm">
        <div className="mb-6 text-center">
          ...
          <h2 className="mt-4 text-lg font-semibold text-stone-700">
            NeuraTantraAI Oversight Portal
          </h2>
          <p className="mt-1 text-sm text-stone-400">
            Enter your internal oversight passcode to access the monitoring dashboard
          </p>
        </div>
